...
// wrapc file define:
#define AUTOTB_TVIN_output "../tv/cdatafile/c.nn_inference.autotvin_output_r.dat"
#define AUTOTB_TVOUT_output "../tv/cdatafile/c.nn_inference.autotvout_output_r.dat"
...
// tvout file define:
#define AUTOTB_TVOUT_PC_output "../tv/rtldatafile/rtl.nn_inference.autotvout_output_r.dat"
...
extern "C" void apatb_nn_inference_hw(volatile void * __xlx_apatb_param_input_img, volatile void * __xlx_apatb_param_output) {
  ...
  if (wrapc_switch_file_token.good())
  {
    ...
    string AESL_num;{
      ...
      if (!rtl_tv_out_file.is_open()) {
        rtl_tv_out_file.open(AUTOTB_TVOUT_PC_output);
      ...
      if (rtl_tv_out_file.good()) {
        ...
        if (atoi(AESL_num.c_str()) == AESL_transaction_pc) {
          std::vector<sc_bv<32> > output_pc_buffer(2);
          int i = 0;
          ...
          while (AESL_token != "[[/transaction]]"){
            ...
            if (AESL_token != "") {
              output_pc_buffer[i] = AESL_token.c_str();;
              i++;
            }
          ...
          if (i > 0) {{
            ...
            ((int*)__xlx_apatb_param_output)[j] = output_pc_buffer[i].to_int64();
          }}}
        } // end transaction
      } // end file is good
...
sc_bv<32> __xlx_tmp_lv = ((int*)__xlx_apatb_param_input_img)[j];
  ...
  tcl_file.set_num(1620, &tcl_file.input_img_depth);
...
sc_bv<32> __xlx_tmp_lv = ((int*)__xlx_apatb_param_output)[j];
  ...
  tcl_file.set_num(2, &tcl_file.output_depth);
  sprintf(__xlx_sprintf_buffer.data(), "[[/transaction]] \n");
  aesl_fh.write(AUTOTB_TVIN_output, __xlx_sprintf_buffer.data());
}
CodeState = CALL_C_DUT;
nn_inference_hw_stub_wrapper(__xlx_apatb_param_input_img, __xlx_apatb_param_output);
CodeState = DUMP_OUTPUTS;
// print output Transactions
{
  sprintf(__xlx_sprintf_buffer.data(), "[[transaction]] %d\n", AESL_transaction);
  aesl_fh.write(AUTOTB_TVOUT_output, __xlx_sprintf_buffer.data());
  {  __xlx_offset_byte_param_output = 0*4;
  if (__xlx_apatb_param_output) {
    for (int j = 0  - 0, e = 2 - 0; j != e; ++j) {
sc_bv<32> __xlx_tmp_lv = ((int*)__xlx_apatb_param_output)[j];
